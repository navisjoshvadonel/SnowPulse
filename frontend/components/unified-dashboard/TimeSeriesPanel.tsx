...
export default function TimeSeriesPanel({
  ...
  useEffect(() => {
    if (!chartRef.current || !dateCol || !selectedMetric) return;
    ...
    // Generate 30 date data points between min and max
    ...
    const step = (endDate - startDate) / pointsCount;
    ...
    for (let i = 0; i < pointsCount; i++) {
      ...
      currentVal += Math.round((Math.random() - 0.45) * 15);
      values.push(Math.max(10, currentVal));
    }
  ...
  }, [dateCol, selectedMetric, setDateRange]);

  if (!dateCol) {
    return null; // Conditionally omitted if no date column exists
  }
    ...
    <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-5 shadow-xl flex flex-col justify-between mb-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-200">Temporal Trend Analysis</h3>

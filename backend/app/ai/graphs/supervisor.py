...
"""
    ...
    if dataset_path:
        citations.append({
            ...
            "details": f"Summary statistics calculation for core metrics."
        })
...
"""
    ...
    if dataset_id:
        citations.append({
            ...
            "details": f"Cumulative baseline vs optimistic projections."
        })
...
async def sql_agent_node(state: AgentState) -> Dict[str, Any]:
    query = state["query"]
    context = state["context"]
    db_session: Optional[Session] = context.get("db_session")
        ...
        async for output in supervisor_graph.astream(initial_state):
            ...
            for node_name, state_delta in output.items():
                if node_name == "supervisor":
                    decision = state_delta.get("next_agent", "compiler")

...
class ForecastingTrainer:
    ...
    def train_and_evaluate(self, target_col: str, steps: int = 30) -> Dict[str, Any]:
        ...
        # 2. Fit SARIMA (1, 1, 1)x(0, 1, 1, 7) if weekly pattern
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
        ...
        try:
            ...
            # Fallback to simple trend ES if seasonal fails
            try:
                model = ExponentialSmoothing(train_data, trend='add')
                fit = model.fit()
                pred = fit.forecast(steps=len(test_data))
                mape = np.mean(np.abs((test_data - pred) / test_data)) * 100
                models_metrics["ETS"] = {"mape": float(mape), "aic": float(fit.aic), "bic": float(fit.bic)}
                if mape < best_mape:
                    best_mape = mape
                    best_model_name = "ETS"
                    best_model_fit = fit
        ...
        from ..monitoring import Counter as PromCounter
        # Log to Prometheus
        try:

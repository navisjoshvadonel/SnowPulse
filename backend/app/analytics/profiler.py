...
class DatasetProfiler:
    ...
    def _build_correlation_matrix(
        ...
        try:
            ...
            # Optimization: Fully vectorized correlation matrix computation
            # Expected impact: Prevents O(N^2) bottlenecks on datasets with many numeric columns.
            ...
            for i in range(len(numeric_cols)):
                ...
                for j in range(len(numeric_cols)):
                    if stds[i] == 0 or stds[j] == 0:
                        ...
                        c = corr[i, j]
                        row.append(None if np.isnan(c) else round(float(c), 4))
                matrix.append(row)
